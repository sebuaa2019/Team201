...
/*
struct{
	int num;            //Object detected quantity 
	BoundingBox bounding_boxes[];
}BoundingBoxes

struct{
	String Class;         //Object category
	float64 posibilities; //posibility that object is in Class
	int64 xmin;           //object left end x coordinate
	int64 xmax;			  //object right end y coordinate
	int64 ymin;			  //object left end x coordinate
	int64 ymax;			  //object right end y coordinate
}BoundingBox
*/
...
void change_msg(int object_num,String Class[],int xmin[]){//实际上用到的属性只有这三个，所以就只需要改变这三个参数 
	msg.num = num;
	int i=0;
	for(;i<num;i++){
